...
const showPassword = ref(false) // 🚀 NUEVO: Variable para el ojito

function selectUser(user) {
  selectedUser.value = user
  password.value = ''
  errorMsg.value = ''
  showPassword.value = false // Reseteamos el ojito al cambiar de usuario
}
...
<template>
  <div class="min-h-screen bg-orange-50 flex items-center justify-center p-4">
    ...
    <div
      class="bg-white p-8 rounded-2xl w-full max-w-md text-center border border-orange-100 shadow-xl transition-all duration-300">

      <div
        class="text-orange-500 mb-4 bg-orange-100 w-20 h-20 rounded-full flex items-center justify-center mx-auto shadow-inner">
      ...
      <div v-if="!selectedUser" class="grid grid-cols-2 gap-4">
        <button v-for="user in users" :key="user.id" @click="selectUser(user)"
          ...
          <div class="mb-2 group-hover:scale-110 transition duration-300">
            <svg v-if="user.id === 'GloriaNora'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
              stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M11.48 3.499a.562.562 0 0 1 1.04 0l2.125 5.111a.563.563 0 0 0 .475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 0 0-.182.557l1.285 5.385a.562.562 0 0 1-.84.61l-4.725-2.885a.562.562 0 0 0-.586 0L6.982 20.54a.562.562 0 0 1-.84-.61l1.285-5.386a.562.562 0 0 0-.182-.557l-4.204-3.602a.562.562 0 0 1 .321-.988l5.518-.442a.563.563 0 0 0 .475-.345L11.48 3.5Z" />
            </svg>
            <svg v-else-if="user.id === 'admin'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
              stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
            </svg>
            <svg v-else-if="user.id === 'Usuarios'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
              stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
              <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <div v-else class="animate-in fade-in zoom-in duration-200">
        <div
          class="flex flex-col items-center justify-center gap-1 mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100">
          <div class="text-center">
            <p class="text-sm text-slate-500 leading-tight mb-1">Ingresando como:</p>
        ...
        <form @submit.prevent="login" class="space-y-4">
          <div class="relative">
            ...
            <input v-model="password" :type="showPassword ? 'text' : 'password'" placeholder="••••••••"
              class="w-full bg-white border-2 border-gray-200 rounded-xl py-4 pl-12 pr-12 text-center text-2xl tracking-widest text-slate-800 focus:border-orange-500 focus:outline-none transition shadow-inner disabled:bg-gray-50"
            ...
            <button type="button" @click="togglePasswordVisibility" 
              ...
              tabindex="-1"> <svg v-if="showPassword" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
          ...
          <button type="button" @click="cancelLogin" :disabled="isLoading"
