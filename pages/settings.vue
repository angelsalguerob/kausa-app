  ...
  <div class="min-h-screen bg-gray-50 flex flex-col lg:flex-row pb-20 lg:pb-0">
    ...
    <aside class="w-full lg:w-64 bg-white border-r border-gray-200 shrink-0 lg:h-screen lg:sticky top-0 p-4 overflow-y-auto">
      ...
      <nav class="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 scrollbar-hide">
        ...
        <button @click="activeTab = 'usuarios'" class="flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors whitespace-nowrap" :class="activeTab === 'usuarios' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'">
        ...
        <button @click="activeTab = 'mapa'" class="flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors whitespace-nowrap" :class="activeTab === 'mapa' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'">
        ...
        <button @click="activeTab = 'impresion'" class="flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors whitespace-nowrap" :class="activeTab === 'impresion' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0 1 10.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0 .229 2.523a1.125 1.125 0 0 1-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0 0 21 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 0 0-1.913-.247M6.34 18H5.25A2.25 2.25 0 0 1 3 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 0 1 1.913-.247m10.5 0a48.536 48.536 0 0 0-10.5 0v2.796c0 .112.08.202.19.202h10.12c.11 0 .19-.09.19-.202V8.987Zm-8.69 4.043h.008v.008H8.69v-.008Zm3 0h.008v.008h-.008v-.008Zm3 0h.008v.008h-.008v-.008Z" /></svg>
          Tickets e Impresión
        </button>
        <button @click="activeTab = 'avanzado'" class="flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors whitespace-nowrap" :class="activeTab === 'avanzado' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'">
    ...
    <main class="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto">
      ...
      <div v-if="activeTab === 'mapa'">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm gap-4">
          <div>
          ...
          <div class="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
              <Transition
                ...
                enter-from-class="opacity-0 w-0 -translate-x-4"
                enter-to-class="opacity-100 w-auto translate-x-0"
                leave-active-class="transition-all duration-200 ease-in"
                leave-from-class="opacity-100 w-auto translate-x-0"
                leave-to-class="opacity-0 w-0 -translate-x-4"
              >
                <button 
                  v-if="selectedTable"
                  @click="promptDeleteTable"
              ...
              >
                <svg v-if="!isSaving" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
        ...
        <div class="bg-white p-4 rounded-3xl border border-gray-200 shadow-lg relative flex flex-col">
          <div class="flex items-center justify-between mb-4 px-3 pt-2">
              <p class="text-sm text-slate-500 font-medium">Arrastra las mesas para organizarlas en los diferentes ambientes.</p>
              <span class="text-xs font-black text-slate-400 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">Total Mesas: {{ store.tables.filter(t => t.type === 'table').length }}</span>
          </div>

          <div v-if="isLoading" class="w-full min-h-[500px] bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 shadow-inner">
          ...
          <div v-else class="w-full overflow-x-auto bg-slate-50 rounded-2xl border border-gray-200 shadow-inner cursor-crosshair">
            <div 
                ref="canvasRef"
            ...
            >
              <div class="absolute top-0 left-0 w-[200px] h-[250px] bg-slate-200 border-[3px] border-slate-800 flex items-center justify-center pointer-events-none z-0">
                <span class="font-black text-slate-500 uppercase tracking-widest text-lg">Cocina</span>
              </div>

              <div class="absolute top-[250px] left-0 w-[150px] h-[300px] border-[3px] border-slate-800 flex items-center justify-center pointer-events-none z-0 bg-white">
                <span class="font-black text-red-500 text-center uppercase tracking-widest leading-loose">Area<br>de<br>pedi<br>dos</span>
              </div>

              <div class="absolute top-[550px] left-0 w-[200px] h-[250px] border-[3px] border-slate-800 flex items-center justify-center pointer-events-none z-0 bg-white">
                <span class="font-black text-red-500 text-xl uppercase tracking-widest">Baños</span>
              </div>

              <div class="absolute top-0 left-[200px] w-[650px] h-[250px] flex flex-col items-center justify-center pointer-events-none z-0 bg-transparent">
              ...
              <div class="absolute top-[250px] left-[150px] w-[700px] h-[300px] bg-white border-[3px] border-slate-800 flex items-center justify-center pointer-events-none z-0">
                <span class="font-black text-slate-200 text-4xl uppercase tracking-widest">Área Principal</span>
              </div>

              <div class="absolute top-[380px] left-[830px] w-[40px] h-[40px] bg-white border-2 border-slate-800 flex items-center justify-center pointer-events-none z-10">
                <span class="font-black text-[8px] uppercase tracking-widest rotate-90 text-slate-800">Puerta</span>
              </div>

              <div class="absolute top-[50px] left-[850px] w-[300px] h-[700px] bg-emerald-50/20 border-[3px] border-slate-800 flex items-center justify-center pointer-events-none z-0">
                <span class="font-black text-emerald-200 text-6xl uppercase tracking-widest rotate-90 opacity-50 absolute">Patio</span>
                <span class="font-black text-red-500 text-xl uppercase tracking-widest absolute right-8">Caja</span>
              </div>

              <div class="absolute top-[550px] left-[200px] w-[300px] h-[250px] bg-orange-50 border-[3px] border-slate-800 flex items-center justify-center pointer-events-none z-0">
                <span class="font-black text-orange-300 uppercase tracking-widest text-xl">Área Exclusiva</span>
              </div>

              <div class="absolute top-[550px] left-[500px] w-[350px] h-[250px] flex items-center justify-center pointer-events-none z-0 bg-transparent">
                <span class="font-black text-red-500 text-lg uppercase tracking-widest text-center">Acceso tienda y<br>2do piso</span>
              </div>

              <div 
                v-for="mesa in store.tables.filter(t => t.type === 'table')" 
                :key="mesa.id"
      ...
      <div v-if="activeTab === 'general'" class="max-w-2xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-5">
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Nombre del Restaurante</label>
            <input v-model="config.nombre" type="text" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition">
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">RUC</label>
              <input v-model="config.ruc" type="text" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition">
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Moneda Oficial</label>
              <select v-model="config.moneda" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition appearance-none">
          ...
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Dirección del Local</label>
            <input v-model="config.direccion" type="text" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition">
          </div>
          <div class="pt-4 border-t border-gray-100">
            <button @click="saveMockSettings" class="bg-slate-800 hover:bg-slate-900 text-white font-bold py-3 px-6 rounded-xl transition shadow-md">Guardar Cambios</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'impresion'" class="max-w-2xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Impresora de Caja (Boletas)</label>
            <select v-model="config.impresoraCaja" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition">
          ...
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Impresora de Cocina (Comandas)</label>
            <select v-model="config.impresoraCocina" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition">
          ...
          <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Mensaje al pie del Ticket de Cliente</label>
            <textarea v-model="config.ticketMsj" rows="3" class="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-500 font-bold transition resize-none"></textarea>
            <p class="text-[10px] text-slate-400 mt-1">Ideal para poner la clave del WiFi o agradecer la visita.</p>
          </div>
          <div class="pt-4 border-t border-gray-100">
            <button @click="saveMockSettings" class="bg-slate-800 hover:bg-slate-900 text-white font-bold py-3 px-6 rounded-xl transition shadow-md">Guardar Ajustes</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'avanzado'" class="max-w-2xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
          ...
          <div>
            ...
            <select v-model="config.botPersonality" class="w-full bg-slate-50 border border-gray-200 text-indigo-800 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 font-bold transition appearance-none">
      ...
      <div v-if="activeTab === 'usuarios'" class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
          <h1 class="text-2xl font-bold text-slate-800">Equipo de Trabajo</h1>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div class="p-4 border-b border-gray-100 flex items-center justify-between hover:bg-gray-50 transition cursor-pointer">
            ...
            <span class="bg-emerald-100 text-emerald-700 px-2 py-1 rounded text-[10px] font-black uppercase">PIN Activo</span>
          </div>

          <div class="p-4 border-b border-gray-100 flex items-center justify-between hover:bg-gray-50 transition cursor-pointer">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-slate-100 text-slate-600 rounded-full flex items-center justify-center font-black">M1</div>
              <div>
                <p class="font-bold text-slate-800 leading-tight">Mesero Turno 1</p>
                <p class="text-xs text-slate-500 font-bold uppercase tracking-wider">Salón</p>
              </div>
            </div>
            <span class="bg-slate-100 text-slate-500 px-2 py-1 rounded text-[10px] font-black uppercase">Sin PIN</span>
          </div>
          ...
          <div class="p-4 flex items-center justify-between hover:bg-gray-50 transition cursor-pointer">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="font-bold text-slate-800 leading-tight">Chef Principal</p>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-2xl border border-red-100 shadow-2xl w-full max-w-sm p-8 text-center">
        <div class="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4 text-red-500 border-4 border-red-100 shadow-inner">
        ...
        <p class="text-slate-500 mb-6 text-sm">Vas a eliminar la <span class="text-slate-800 font-bold">"{{ selectedTable?.name }}"</span> del plano permanentemente.</p>
        <div class="flex gap-3">
           ...
           <button @click="confirmDeleteTable" class="flex-1 bg-red-500 hover:bg-red-600 text-white py-3 rounded-xl shadow-lg shadow-red-500/20 transition font-bold active:scale-95">Sí, quitar</button>
        </div>
      </div>
    </div>
    ...
    >
      <div v-if="showToast" class="fixed bottom-5 right-5 z-[300] bg-white border-l-4 border-emerald-500 text-slate-800 px-6 py-4 rounded-lg shadow-xl flex items-center gap-4 max-w-sm border border-gray-100">
