...
// LÓGICA DE CANCELACIÓN DEL CHEF (AHORA "RECHAZAR")
...
function confirmCancelOrder() {
  ...
  // A) Lo metemos a la lista negra temporal
      ...
      return {
        ...
        subItems: subItems.map(sub => {
          ...
          return {
            ...
            isSin: sub.toLowerCase().startsWith('sin ') // 🚀 NUEVO: Detecta si empieza con "Sin "
          }
        })
      }
    } else {
  ...
  <div class="min-h-[calc(100vh-80px)] bg-gray-50 p-4 md:p-6 relative">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
      <div v-for="order in orders" :key="order.id"
        class="bg-white rounded-2xl shadow-sm border-2 overflow-hidden flex flex-col hover:shadow-lg transition duration-300 relative group"
        :class="{
        ...
        <div v-if="order.status === 'Cancelado'"
          class="bg-red-500 text-white font-black text-center py-2 text-sm uppercase tracking-widest flex items-center justify-center gap-2 animate-pulse">
        ...
        <div class="p-3 md:p-4 border-b flex flex-col gap-3 md:gap-4" :class="{
          ...
          <div class="flex justify-between items-start">
            ...
            <div class="flex items-center gap-2 md:gap-3">
              <div
                class="p-2 md:p-2.5 bg-white rounded-xl shadow-sm border border-black/5 flex items-center justify-center">
                <svg v-if="order.table?.toLowerCase().includes('llevar')" xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"
                  class="w-5 h-5 md:w-6 md:h-6 text-teal-600">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
                </svg>
                <svg v-else-if="getTicketColor(order.description) === 'bebida'" xmlns="http://www.w3.org/2000/svg"
                ...
                <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                  stroke="currentColor" class="w-5 h-5 md:w-6 md:h-6 text-orange-500">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M6.082 18h11.836l1.264-12A1.125 1.125 0 0 1 20.302 4.5H3.698a1.125 1.125 0 0 1 1.12 1.243l1.264 12ZM8.25 10.5h7.5M8.25 14.25h7.5M12 21.75V18M12 4.5v-1.5M12 4.5H9.75M12 4.5h2.25" />
                </svg>
              </div>

              <div>
                <span class="block text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-500">Orden
                  #</span>
                <span class="text-2xl md:text-3xl font-black text-slate-800 leading-none"
                  :class="{ 'text-red-600 line-through opacity-80': order.status === 'Cancelado' }">
            ...
            <div class="flex flex-col items-end gap-1">
              <button v-if="order.status !== 'Cancelado'" @click="askToCancel(order)"
                class="text-slate-400 hover:text-red-500 active:bg-red-100 active:text-red-600 hover:bg-red-50 rounded-lg p-1.5 transition-all -mr-1"
              ...
              <div
                class="flex items-center gap-1 text-slate-500 bg-white/60 px-1.5 py-0.5 md:px-2 md:py-1 rounded border border-black/5 shadow-sm"
                :class="{ 'text-red-500 bg-red-100 border-red-200': order.status === 'Cancelado' }">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                  stroke="currentColor" class="w-3 md:w-3.5 h-3 md:h-3.5">
                  <path stroke-linecap="round" stroke-linejoin="round"
          ...
          <div
            class="w-full bg-white px-3 md:px-4 py-2 md:py-3 rounded-xl shadow-sm border flex items-center justify-center text-center mt-1"
            :class="{
        ...
        <div class="p-3 md:p-4 flex-1 bg-white"
          :class="{ 'opacity-50 grayscale pointer-events-none': order.status === 'Cancelado' }">
          <ul class="space-y-2 md:space-y-3 mb-2">

            <li v-for="(item, idx) in getOrderItems(order.description)" :key="idx">

              <div v-if="item.isCombo"
                class="flex flex-col gap-2 md:gap-3 p-3 md:p-4 rounded-xl border transition-all shadow-sm"
                :class="item.isEjecutivo ? 'bg-indigo-50/40 border-indigo-200' : 'bg-orange-50/40 border-orange-200'">
                <div class="font-black flex items-center gap-2 text-sm md:text-base"
                  :class="item.isEjecutivo ? 'text-indigo-800' : 'text-orange-800'">
                  <span class="relative flex h-2.5 w-2.5 md:h-3 md:w-3">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75"
                      :class="item.isEjecutivo ? 'bg-indigo-400' : 'bg-orange-400'"></span>
                    <span class="relative inline-flex rounded-full h-2.5 w-2.5 md:h-3 md:w-3"
                      :class="item.isEjecutivo ? 'bg-indigo-500' : 'bg-orange-500'"></span>
                  </span>
                  {{ item.text }}
                </div>

                <ul class="pl-3 md:pl-4 space-y-1.5 md:space-y-2 border-l-2 ml-1"
                  :class="item.isEjecutivo ? 'border-indigo-300' : 'border-orange-300'">
                  <li v-for="(sub, sIdx) in item.subItems" :key="sIdx"
                    class="text-xs md:text-sm font-bold flex items-center gap-2 text-slate-700">

                    <span v-if="!sub.isSin" class="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full"
                      :class="item.isEjecutivo ? 'bg-indigo-500' : 'bg-orange-500'"></span>
                    <span v-else class="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-slate-300"></span>

                    <span :class="sub.isSin ? 'text-slate-400 italic font-medium' : 'text-slate-800'">{{ sub.text
                      }}</span>

                    <span v-if="sub.category === 'Bebida' && !sub.isSin"
                      class="ml-auto text-[9px] md:text-[10px] font-black uppercase tracking-widest bg-blue-100 text-blue-700 px-1.5 md:px-2 py-0.5 md:py-1 rounded-md border border-blue-200">Barra</span>
                  </li>
                </ul>
              </div>

              <div v-else
                class="flex items-center gap-2 md:gap-3 p-2.5 md:p-3 rounded-xl text-xs md:text-sm font-bold border transition-all"
                :class="{
                ...
                <div class="relative bg-white p-1 md:p-1.5 rounded-lg shadow-sm border border-black/5 flex-shrink-0">
                  <span v-if="item.category === 'Comida'"
                    class="absolute inset-0 rounded-lg bg-orange-500 opacity-40 animate-ping"></span>
                  <svg v-if="item.category === 'Comida'" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                    class="relative w-3.5 h-3.5 md:w-4 md:h-4 text-orange-600">
                    <path stroke-linecap="round" stroke-linejoin="round"
                      d="M15.362 5.214A8.252 8.252 0 0 1 12 21 8.25 8.25 0 0 1 6.038 7.047 8.287 8.287 0 0 0 9 9.601a8.983 8.983 0 0 1 3.361-6.867 8.21 8.21 0 0 0 3 2.48Z" />
                  </svg>
                  <svg v-else-if="item.category === 'Bebida'" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                    class="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-500">
                    <path stroke-linecap="round" stroke-linejoin="round"
                      d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" />
                  </svg>
                  <svg v-else-if="item.category === 'Postre'" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                    class="w-3.5 h-3.5 md:w-4 md:h-4 text-pink-500">
                    <path stroke-linecap="round" stroke-linejoin="round"
                  ...
                  <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                    stroke="currentColor" class="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-500">
                    <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <span v-if="item.category === 'Bebida'"
                  class="ml-auto text-[9px] md:text-[10px] font-black uppercase tracking-widest bg-blue-100 text-blue-700 px-1.5 md:px-2 py-0.5 md:py-1 rounded-md border border-blue-200">Barra</span>
              </div>
            </li>
          </ul>
        ...
        <div class="p-3 md:p-4 bg-slate-50 border-t border-slate-200 mt-auto">
          <button v-if="order.status !== 'Cancelado'" @click="markAsReady(order.id)"
            class="w-full border-2 font-bold py-2.5 md:py-3 rounded-xl transition flex items-center justify-center gap-2 shadow-sm text-sm md:text-base"
            :class="{
    ...
    <div v-if="orders.length === 0"
      class="flex flex-col items-center justify-center py-16 md:py-20 text-slate-400 opacity-80 text-center">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor"
        class="w-24 h-24 md:w-32 md:h-32 mb-4 text-slate-300">
        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div v-if="showCancelModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"
        @click="!isCancelling && closeCancelModal()"></div>
      <div
        class="relative bg-white rounded-3xl border border-red-100 shadow-2xl max-w-sm w-full p-6 md:p-8 text-center transform transition-all">
        <div
          class="w-16 h-16 md:w-20 md:h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6 text-red-500 shadow-inner border-4 border-red-100">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
            stroke="currentColor" class="w-8 h-8 md:w-10 md:h-10">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3Z" />
          </svg>
        </div>
        <h3 class="text-xl md:text-2xl font-black text-slate-800 mb-2">¿Eliminar Orden #{{ orderToCancel?.dailyTicket ||
          orderToCancel?.id }}?</h3>
        <p class="text-slate-500 mb-6 text-xs md:text-sm">Estás a punto de rechazar el pedido de la <strong
            class="text-slate-700">{{ orderToCancel?.table || 'Caja' }}</strong>. Desaparecerá de la pantalla.</p>
        <div class="flex gap-3">
          <button @click="closeCancelModal" :disabled="isCancelling"
            class="flex-1 bg-white border-2 border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 active:bg-slate-100 font-bold py-2.5 md:py-3 rounded-xl transition shadow-sm text-sm md:text-base">Atrás</button>
          <button @click="confirmCancelOrder" :disabled="isCancelling"
            class="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-2.5 md:py-3 rounded-xl shadow-lg transition active:scale-95 flex items-center justify-center text-sm md:text-base">
